...
const dictionary = Dictionary("/dictionary.json");
...
async function possibleWords(availableTiles) {
  ...
  const dictResult = await dictionary.get();

  if (dictResult.status === "success") {
    const dictWords = dictResult.data;
    ...
    console.error(dictResult.message, dictResult.error);
  }
...
async function isValid(word) {
  ...
  if (dictResult.status === "success") {
    ...
    for (let i = 0; i < 26; ++i) {
      ...
      // replace only replaces the first occurrence of *.
